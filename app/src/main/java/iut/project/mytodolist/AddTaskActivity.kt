...
class AddTaskActivity : AppCompatActivity() {
    ...
    //method for saving records in the database
    var idCounter = 0
    fun saveRecord(view: View) {
        ...
        val isHighPriority = findViewById<CheckBox>(R.id.checkbox_priority).isChecked
        ...
        if (name.trim() != "" && description.trim() != "") {
            val status = databaseHandler.addTask(
                TaskModelClass(
                    ...
                    description,
                    if (noDate) "" else date,
                    if (isHighPriority) 1 else 0 // Set priority to 1 if checkbox is checked, 0 otherwise
                )
            )
            if (status > -1) {
