...
class MainActivity : AppCompatActivity() {
    ...
    fun updateRecord(view: View) {
        ...
        dialogBuilder.setTitle("Task editing")
        dialogBuilder.setMessage("Modify the desired fields")
        dialogBuilder.setPositiveButton("Modifier", DialogInterface.OnClickListener { _, _ ->
            ...
            if (updateName.trim() != "" && updateDescription.trim() != "") {
                ...
                if (status > -1) {
                    Toast.makeText(applicationContext, "Successful modification", Toast.LENGTH_LONG)
                        .show()
                    viewRecord()
                }
            } else {
    ...
    fun deleteRecord(view: View) {
        ...
        if (!done) {
            ...
            alertDialogBuilder.setPositiveButton("delete") { dialog, which ->
                ...
                if (status > -1) {
                    Toast.makeText(applicationContext, "successful deletion", Toast.LENGTH_LONG)
                        .show()
                    viewRecord()
                }
            }
